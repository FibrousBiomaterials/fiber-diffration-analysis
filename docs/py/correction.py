"""ローレンツ補正・偏光補正マップを計算するモジュール。

instensity_resrict_polar.ipynb セル17 の [0] 偏光補正係数 / [1] ローレンツ補正+偏光補正
をそのまま移植したもの。強度抽出パイプラインの最初のステップ。
"""
...
def build_lorentz_map(
    ...
    # メモリ確保に失敗することがある。計算結果はオリジナル(逐次にすべて保持する版)と
    # 完全に同じになるよう、使い終わった配列を del するだけで演算の順序・内容は
    # 一切変更していない。
    ny, nx = shape

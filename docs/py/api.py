...
旧 FastAPI 版 main.py のルートハンドラを、HTTP/Pydantic/ジョブポーリングに
依存しないプレーン関数として移植していく。各ハンドラは
...
main.py 時代の「同期エンドポイントは毎回ファイルを再読込・再デコードする」という
挙動は、移行の第一段階ではそのまま踏襲する(呼び出し側 = app.js が毎回
File API から読み直したバイト列を渡してくる)。
"""
...
# JSON化しないので専用のストアに保持する(main.py時代のジョブストアを踏襲)。
...
def _to_preview_png(img: np.ndarray) -> tuple[str, float, int, int]:
    ...
    # ノートブックの cmap="Greys" に合わせ、強度が高いほど黒くなるよう反転
...
def _background_removal(payload: dict, progress_cb, file_bytes) -> dict:
    ...
    # ノートブックのセル17と同じ順序: ローレンツ+偏光補正マップは傾き補正前の
    # 生画像座標系で計算し、傾き補正は極座標リビニングの中でまとめて行う。
    inv_lp, mask = build_lorentz_map(
...
def _block_fit(payload: dict, progress_cb, file_bytes) -> dict:
    ...
    # 配向幅Bは「7. ピーク幅の算出」で別途計算済みの値を使う(ノートブック cell27相当)
    ...
    # --- ブロックごとのNNLSフィット (ノートブック cell38相当) ---
    ...
    #     (ノートブック cell48/49相当)。面間隔が近い別hklとのさらなるマージは
    #     別ルート (block_fit_merge_by_dspacing) で行う。

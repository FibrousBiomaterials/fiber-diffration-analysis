...
処理の流れ:
  有効な反射の抽出 (count>0, intensity>0, sigma_I>0 かつ有限)
  同一hklの反射を重み付き平均でマージ (merge_by_hkl)
  逆格子計量テンソルからのd値計算(既存の逆格子ベクトルの内積で代替)
  同一l内でd値が近い反射(フリーデル対を含む対称等価反射)を重み付き平均でマージ
             (merge_by_dspacing)

この2つは別々の(独立して呼び出せる)ステップとして分けてある。
